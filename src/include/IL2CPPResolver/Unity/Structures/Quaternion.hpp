...
struct Quaternion {
  ...
  inline Quaternion(float data[]) noexcept
      : x(data[0]), y(data[1]), z(data[2]), w(data[3]) {}
  inline Quaternion(Vector3 vector, float scalar) noexcept
  ...
  inline static Quaternion RotateTowards(Quaternion from, Quaternion to,
  ...
  inline static void ToAngleAxis(Quaternion rotation, float &angle,
                                 Vector3 &axis);
  ...
  inline friend Quaternion operator*(const float s, const Quaternion &q) {
    Quaternion t(q);
    return t *= s;
  }
  inline friend Quaternion operator/(const Quaternion &q, const float s) {
    Quaternion t(q);
    return t /= s;
  }
...
inline Vector3 operator*(Quaternion lhs, const Vector3 rhs) {
  ...
  return u * (Vector3::Dot(u, rhs) * 2.0f) +
         rhs * (lhs.w * lhs.w - Vector3::Dot(u, u)) +
         Vector3::Cross(u, rhs) * (2.0f * lhs.w);
}
...
Quaternion Quaternion::FromEuler(float yaw, float pitch, float roll,
  ...
  Simplified multiplication of this data:
    ...
    return (qY * qX) * qZ;
  */
  return {cP * sY * cR + sP * cY * sR, sP * cY * cR - cP * sY * sR,
...
Vector3 Quaternion::ToEuler(Quaternion q, bool toDeg) {
  ...
  if (abs(singularity_test) < 0.499999f) {
    float xx = q.x * q.x, yy = q.y * q.y, yw = q.y * q.w, zz = q.z * q.z,
          ww = q.w * q.w;
...
Quaternion Quaternion::LookRotation(Vector3 forward, Vector3 upwards) {
  ...
  if (Vector3::SqrMagnitude(forward) < smallFloat ||
      Vector3::SqrMagnitude(upwards) < smallFloat)
    return Quaternion::identity;
  if (1 - fabs(Vector3::Dot(forward, upwards)) < smallFloat)
...
float Quaternion::Norm(Quaternion rotation) {
  return sqrtf(rotation.x * rotation.x + rotation.y * rotation.y +
...
Quaternion Quaternion::SlerpUnclamped(Quaternion a, Quaternion b, float t) {
  ...
  if (dot < 0.95f) {
    ...
    return (a * sinf(angle * (1.0f - t)) + b * sinf(angle * t)) *
           (1.0f / sinf(angle));
  }
  return LerpUnclamped(a, b, t);
}
...
inline Vector3 Quaternion::RotateVectorByQuaternion(Quaternion lhs,
                                                    Vector3 rhs) {
  float x = lhs.x * 2.f;
  float y = lhs.y * 2.f;
  float z = lhs.z * 2.f;
