...
struct Vector3 {
  ...
  inline static float Component(Vector3 a, Vector3 b) {
  ...
  inline static Vector3 Lerp(Vector3 from, Vector3 to, float t) {
    return to * t + from * (1.0f - t);
  }
  inline static Vector3 LerpUnclamped(Vector3 from, Vector3 to, float t) {
  ...
  inline static Vector3 MoveTowards(Vector3 current, Vector3 target,
  ...
  inline static void OrthoNormalize(Vector3 &normal, Vector3 &tangent,
                                    Vector3 &binormal);
  inline static void OrthoNormalizeFast(Vector3 &normal, Vector3 &tangent,
                                        Vector3 &binormal);
  ...
  inline static Vector3 RotateTowards(Vector3 current, Vector3 target,
  ...
  inline static Vector3 SmoothDamp(Vector3 current, Vector3 target,
                                   Vector3 &currentVelocity, float smoothTime,
                                   float maxSpeed, float deltaTime);
  inline static float SqrMagnitude(Vector3 vector) {
    return vector.x * vector.x + vector.y * vector.y + vector.z * vector.z;
  }
  inline static void ToSpherical(Vector3 vector, float &rad, float &theta,
                                 float &phi);
  ...
  inline Vector3 normalizedEuler(bool is180 = true) const {
    return NormalizeEuler(*this, is180);
  }
  ...
  inline Vector3 &operator+=(Vector3 inV) {
    x += inV.x;
    y += inV.y;
    z += inV.z;
    return *this;
  ...
  inline friend Vector3 operator+(Vector3 lhs, Vector3 rhs) {
    return {lhs.x + rhs.x, lhs.y + rhs.y, lhs.z + rhs.z};
  }
  inline friend Vector3 operator-(Vector3 lhs, Vector3 rhs) {
    return {lhs.x - rhs.x, lhs.y - rhs.y, lhs.z - rhs.z};
  }
  inline friend Vector3 operator*(Vector3 v, float s) {
    return {v.x * s, v.y * s, v.z * s};
  }
  inline friend Vector3 operator*(float s, Vector3 v) {
    return {v.x * s, v.y * s, v.z * s};
  }
  inline friend Vector3 operator*(Vector3 lhs, Vector3 rhs) {
    return {lhs.x * rhs.x, lhs.y * rhs.y, lhs.z * rhs.z};
...
float Vector3::NormalizeAngle(float f, bool is180) {
  while (f > (is180 ? 180.f : 360.f))
    f -= 360.f;
  while (f < (is180 ? -180.f : 0.f))
    f += 360.f;
  return f;
}
...
Vector3 Vector3::Orthogonal(Vector3 vector) {
  return vector.z < vector.x ? Vector3(vector.y, -vector.x, 0)
                             : Vector3(0, -vector.z, vector.y);
}
...
Vector3 Vector3::RotateTowards(Vector3 current, Vector3 target,
  ...
  float newMag = magCur + maxMagnitudeDelta *
                              (float)((magTar > magCur) - (magCur > magTar));
...
float Vector3::SignedAngle(Vector3 from, Vector3 to, Vector3 axis) {
  ...
  float sign = (axis.x * cross_x + axis.y * cross_y + axis.z * cross_z) >= 0.f
                   ? 1.f
                   : -1.f;
...
Vector3 Vector3::SmoothDamp(Vector3 current, Vector3 target,
                            Vector3 &currentVelocity, float smoothTime,
                            float maxSpeed, float deltaTime) {
  ...
  if (origMinusCurrent.x * outMinusOrig.x +
          origMinusCurrent.y * outMinusOrig.y +
          origMinusCurrent.z * outMinusOrig.z >
      0.f) {
