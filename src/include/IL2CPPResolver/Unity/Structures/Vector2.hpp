...
struct Vector2 {
  ...
  inline static float Component(Vector2 a, Vector2 b) {
  ...
  inline static Vector2 Lerp(Vector2 from, Vector2 to, float t) {
    return to * t + from * (1.0f - t);
  }
  inline static Vector2 LerpUnclamped(Vector2 from, Vector2 to, float t) {
  ...
  inline static Vector2 MoveTowards(Vector2 current, Vector2 target,
  ...
  inline static Vector2 Perpendicular(Vector2 inDirection) {
  ...
  inline static Vector2 SmoothDamp(Vector2 current, Vector2 target,
                                   Vector2 &currentVelocity, float smoothTime,
                                   float maxSpeed, float deltaTime);
  inline static float SqrMagnitude(Vector2 vector) {
  ...
  inline friend Vector2 operator+(Vector2 lhs, float rhs) {
  ...
  inline friend Vector2 operator+(float lhs, Vector2 rhs) {
  ...
  inline friend Vector2 operator+(Vector2 lhs, Vector2 rhs) {
    return {lhs.x + rhs.x, lhs.y + rhs.y};
  }
  inline friend Vector2 operator-(Vector2 lhs, Vector2 rhs) {
    return {lhs.x - rhs.x, lhs.y - rhs.y};
...
Vector2 Vector2::Reject(Vector2 a, Vector2 b) { return a - Project(a, b); }
...
Vector2 Vector2::RotateTowards(Vector2 current, Vector2 target,
  ...
  if (1 - fabsf(axis) >= 0.00001)
    axis = 1;
  current = Normalize(current);
  Vector2 newVector =
      current * cosf(maxRadiansDelta) +
      Vector2(-current.y, current.x) * sinf(maxRadiansDelta) * axis;
...
Vector2 Vector2::SmoothDamp(Vector2 current, Vector2 target,
                            Vector2 &currentVelocity, float smoothTime,
                            float maxSpeed, float deltaTime) {
